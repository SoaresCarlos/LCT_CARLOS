...
unsigned long sampleTime = 1000;  // 1 segundo para cálculo do PID
...
void setup() {
  ...
  // Recebe as configurações do Python (intervalo de amostragem, número de amostras, temperatura alvo, kp, ki, kd)
...
void loop() {
  ...
  if (millis() - lastTime >= sampleTime) {
    ...
    // Controle PID
    float derivativo = (erro - erroAnterior) / (sampleTime / 1000.0);  // Parte derivativa
    float output = kp * erro + ki * erroAcumulado + kd * derivativo;
    pwmValue = constrain(output, 0, 255);

    analogWrite(transistorPin, pwmValue);  // Ajusta o PWM do aquecedor
    erroAnterior = erro;  // Atualiza erro anterior
